...
import sys, os
sys.path.append("C:/AM5000/python-libTestsetup-nysw/")
...
#instruments
import modules.Multimeter.mAtest as Multimeter1
...
#exit()
import scripts.limits_ResultsCheck as Limit_Result
...
for t in range(Total_sensors):
    ...
    """ time.sleep(2)
    df_NAVbom1_1=NAVdb.NavBOMniva5(Partno_Niva1)
    print(df_NAVbom1_1)
    niva1_1list=df_NAVbom1_1['EXPR1'].tolist()
    print(niva1_1list)
    time.sleep(2)
    df_NAVbom1_1.to_csv('navbom3.txt', index=False,  sep="\t")

    niva1list=df_NAVbom1_1['EXPR1'].tolist()
    print("niva1list")
    print(niva1list)
    niva1descriptionlist=df_NAVbom1_1['EXPR2'].tolist()
    print(niva1descriptionlist)
    #exit()
    for i in range (len(niva1descriptionlist)):
        #print (niva1descriptionlist[i].find("Housing"))

        if ("Inlet" in niva1descriptionlist[i]):
            print("inlet")
            print(df_NAVbom1_1['EXPR1'][i])
            
            prodno=df_NAVbom1_1['EXPR1'][i]
            print("prodno-inlet",prodno)
            prodnoinlet=prodno
            #if (prodno==120144):
            
                    
        elif ("Diaph" in niva1descriptionlist[i]):
            print("Diaphragm")
            print(df_NAVbom1_1['EXPR1'][i])
            prodno=df_NAVbom1_1['EXPR1'][i]
            print("prodno-diaph", prodno)
            prodnodiaph=prodno
        ...
        if i==0:
            ...
            #exit()
                ...
                #for i in range (len(dfsp)-1): # ONLT WRITING COMPSERT ONCE
                    ...
                    #print(compsert_Elektronikk)
                    #print(type(compsert_Elektronikk))
                    #exit()
                    ...
                    """
                    if Niva3ProductNo not in SINGLEPARTNO:    
                        ...
                        EEPdata= ','.join(e)
                        #EEPdata=e
                        EEPROM_Prosess_ID=Limit_Result.offsetSQLdb_Process(int(elementid),1,Prosess,ProdOrderx, ProdNox, Station,ProsessType,sw,Batch,int(Operator_ID),Recipe_ID)
                        ...
                        print("NORBIT SERIAL NO -KANAL B",ElekSlnoch2)
                        compsert_Elektronikk=pga.ReadSupplierBatch_prev()
                        print("NORBIT BATCH NO",compsert_Elektronikk)
                        print(compsert_Elektronikk)
                        print(type(compsert_Elektronikk))
                        ...
                        Limit_Result.ElectronicSQLdb_Process(int(elementid), ProdOrderx,ProdNox,SerialNumber,ElekSlnoch2,compsert_Elektronikk,int(Operator_ID),Comments,DateTime,1,Bootloader,application,Kommenter)
                    ...
                    # Common parameters for creating process id
                    ProsessType='Test&Setup'
                    ProdOrderx=int(WO)
