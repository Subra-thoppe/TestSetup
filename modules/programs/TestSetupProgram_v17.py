...
#24,01,25 -v16 jeya -Norbit - Batch and elctronic serail no registers are wrong ( Not got update/ info from waqas . checked self and reading correct registers now)
#29,01,25 - V17 JEYA - NAV NIVA CHECK , ERROR MELDING MED FORKLARING
#13,02,25 - Scansense supplier serial no read regitser was feil . reading from correct regitser now . Adding feil control with supplier batch no er default ==99999
import pyvisa
import time
import ctypes 
...
for t in range(Total_sensors):
    ...
    WO=guiwo.WORKORDER
    #check if WO er Niva3
    WOniva=NAVdb.CheckWOniva('WO'+str(WO))
    if WOniva!='Niva_3':
          ctypes.windll.user32.MessageBoxW(0,f" Hei, WO ser ut fra {WOniva}...  Kan dere sjekke hvis den WO nummer er riktig og Nivå 3 WO ", "Error", 0)
          exit()
    oprname=guiwo.title
    #print(oprname)
    ...
    if len(dfw)!=0:
        ...
        if len(dfc)!=0:
            ...
            ctypes.windll.user32.MessageBoxW(0, " Finner ikke BondeOppsett for denne WO.. Sjekk hvis Pull test verdier er reigistert i Wirebond på den WO ", "Error", 0)
            exit()
        ...
        if i==0:
            ...
            #exit()
                ...
                #for i in range (len(dfsp)-1): # ONLT WRITING COMPSERT ONCE
                    ...
                    ElekSlnoch1= pga.ReadSupplierSerial()
                    print(ElekSlnoch1)
                    if int(ElekSlnoch1) <10000 or int(ElekSlnoch1) >1000000 or int(ElekSlnoch1)==99999:
                        print("setter dummy elektronikk SERIAL no")
                        
                        #ctypes.windll.user32.MessageBoxW(0, " NORBIT  Elektronikk serial no - finner ikke, Kontakt Waqas, Jeya ELLER SKRIV AVVIK PÅ FOU", "Error", 0)
                        ElekSlnoch1=13790
                    print(ElekSlnoch1)
                    ...
                    if (int(compsert_Elektronikk) <10000 or int(compsert_Elektronikk) >1000000 or int(compsert_Elektronikk)==99999):
                        #print("HI")
                        print("setter dummy elektronikk batch no")
                        
                        compsert_Elektronikk=181024
                        print()
                        #ctypes.windll.user32.MessageBoxW(0, " NORBIT  Batch no - finner ikke, Kontakt Waqas, Jeya ELLER SKRIV AVVIK PÅ FOU", "Error", 0)
                    ...
                    if (abs(compsert_Elektronikk) >=1000000 or abs(compsert_Elektronikk)==99999):
                        compsert_Elektronikk=181024
